...
- Phase 2でAIがREQUIREMENTS.mdに基づいて実装を追加
...
- **共通ドメイン実装**: `shared/domain/model/`, `shared/domain/repository/`（EntityとRepository Interfaceのみ）
...
#### REQUIREMENTS.md（必須）
**配置場所**: `composeApp/feature/{feature_name}/REQUIREMENTS.md`
**内容**（3セクション構成）:
1. **ユーザーストーリー**: ユーザー操作と期待する動作を箇条書き
2. **ビジネスルール**: 機能要件、制約条件を箇条書き
3. **画面状態遷移**: Mermaid図で状態遷移を表現

#### テストファイル骨格（必須）
- **ViewModelTest.kt**: 空のテストクラス・メソッド（TODOコメント付き）
- **UseCaseTest.kt**: 複雑なビジネスルールがある場合のみ

**重要**: 詳細な実装仕様（State/Intent定義、DI設定など）はPhase 2でAIが推論します。REQUIREMENTS.mdはあくまで「何を作るか」を明確にする仕様書です。
...
### 5.1 目的
REQUIREMENTS.mdに基づき、AIが実装を実行します。
...
**AI実行方法**:
- Serena Skill: `/serena "Implement based on REQUIREMENTS.md..."`
...
| `docs/design-doc/template/requirements-template.md` | REQUIREMENTS.md | Phase 1 |
...
| **REQUIREMENTS.md** | 詳細実装仕様（AI実装のSSoT） | Phase 1（必須） |
...
**シンプルな仕様書を作成**:
- REQUIREMENTS.mdは3セクション構成（ユーザーストーリー、ビジネスルール、状態遷移）
...
**REQUIREMENTSとテストから仕様を読み取る**:
- REQUIREMENTS.md（ユーザーストーリー、ビジネスルール、状態遷移）
...
- **REQUIREMENTS**: 詳細実装仕様（Phase 1のSSoT、AI実装の直接的な指針）
- **使い分け**: Epic規模の大規模機能はDesign Doc、Story規模はREQUIREMENTS必須

### Issue Contextとの関係
- **Issue Context**: Phase 3（実装後）の記録
- **REQUIREMENTS**: Phase 1（実装前）の仕様
- **統合**: Issue ContextからREQUIREMENTSを参照、実装記録として保存

### ADRとの整合性
- REQUIREMENTS内に「参照ADR」セクションを必ず記載
