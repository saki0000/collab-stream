...
| 4. 実装（エージェント委譲） | → Dev | Shared + UI + テスト + DI |
...
DESIGN.md の「変更対象」セクションからレイヤーを判定し、subagent_type を選択します:

| 変更対象レイヤー | subagent_type |
|----------------|---------------|
| `shared/` のみ | `domain-layer-architect` |
| `composeApp/` のみ（UI中心） | `compose-multiplatform-specialist` |
| `server/` のみ | `kotlin-backend-specialist` |
| 複数レイヤーにまたがる | `general-purpose` |

### 4.3 エージェント起動

Task ツールを呼び出します。prompt は以下のテンプレートに従って構築してください。
`{...}` 部分は実際の値で置換します。

````
あなたは CollabStream プロジェクトの実装エージェントです。
以下の仕様・設計に基づいてコードを実装してください。
...
タスク完了時に PROGRESS.md のチェックボックスを `[x]` に更新してください。
...
1. **Shared Layer**（Domain/Data）
   - Domain Models → Repository Interface → UseCase（必要時） → Repository実装 → テスト
   - 完了時: `./gradlew :shared:build && ./gradlew :shared:test`

2. **ComposeApp Layer**（ViewModel/UI）
   - UiState/Intent → ViewModel（MVI） → UI Components（Route → Screen → Content → Component） → Navigation → ViewModelTest
   - 完了時: `./gradlew :composeApp:build && ./gradlew :composeApp:test`

3. **Integration**
   - DI設定（Koin）: Shared module + ComposeApp module
   - 全テスト: `./gradlew test`

各ステップ完了時にビルド確認コマンドを実行し、エラーがあれば修正してください。
DESIGN.md に Shared Layer の変更がない場合は Step 1 をスキップしてください。
````

### 4.4 エージェント完了後の確認

エージェント完了後、以下を確認します:
1. PROGRESS.md を読み込み、全タスクにチェックが入っているか確認
2. 未完了タスクがある場合は追加の Task を発行して補完
3. 最終ビルド確認: `./gradlew test`
